...
class VariantEffectPredictor:
        ...
        if reference_genome:
            if os.path.exists(reference_genome):
                ...
                raise FileNotFoundError(
                    f"Reference genome not found: {reference_genome}"
                )
    ...
    def predict_variant_effect(
        ...
        required_cols = [
            ...
            alternate_allele_col,
        ]
        missing_cols = [col for col in required_cols if col not in variants_df.columns]

        if missing_cols:
            raise ValueError(
                f"Missing required columns in variants data: {missing_cols}"
            )
        ...
        for idx, row in variants_df.iterrows():
            ...
            try:
                ref_seq, alt_seq = self._get_variant_sequences(
        ...
        result_df = pd.DataFrame(
            {
                ...
                "reference": variants_df[reference_allele_col].values[
                    : len(variant_ids)
                ],
                "alternate": variants_df[alternate_allele_col].values[
                    : len(variant_ids)
                ],
            }
        )
                    ...
                    for col in result_df.columns:
                        if col in ["ref_pred", "alt_pred", "effect"]:
                            ...
                            if data.dtype == np.object:
                                data = np.array(
                                    data, dtype=h5py.special_dtype(vlen=str)
                                )
    ...
    def _get_variant_sequences(
        self, chrom: str, pos: int, ref_allele: str, alt_allele: str
    ) -> Tuple[str, str]:
        """
                ...
                variants.append(
                    {
                        ...
                        "id": (
                            variant.ID
                            if variant.ID
                            else f"{variant.CHROM}:{variant.POS}_{variant.REF}>{variant.ALT[0]}"
                        ),
                    }
                )
    ...
    def _load_bed(self, bed_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            for entry in bed:
                ...
                variants.append(
                    {"chrom": chrom, "pos": pos, "ref": ref, "alt": alt, "id": name}
                )
...
def score_variants(
    ...
    return predictor.predict_variant_effect(
        variants=variants_file, output_file=output_file
    )
        ...
        target_metrics.append(
            {
                ...
                "effect_ratio": (pos_sig + 1)
                / (neg_sig + 1),  # Add 1 to avoid division by zero
            }
        )
