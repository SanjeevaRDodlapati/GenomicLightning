...
@click.option("--gpus", default=None, type=int, help="Number of GPUs to use")
@click.option(
    "--max-epochs", default=None, type=int, help="Maximum number of epochs to train"
)
@click.option(
    "--resume",
    default=None,
...
@click.option("--output-path", "-o", required=True, help="Path to save predictions")
@click.option("--batch-size", default=32, type=int, help="Batch size for prediction")
@click.option("--gpus", default=None, type=int, help="Number of GPUs to use")
def predict(model_path, data_path, output_path, batch_size, gpus):
    """Make predictions with a trained model."""

    predictions = predict_with_model(
        model_path=model_path, data_path=data_path, batch_size=batch_size, devices=gpus
    )
...
@click.option("--output-path", "-o", required=True, help="Path to save converted model")
def convert(legacy_model, model_type, config_path, output_path):
    ...
    model = import_model_from_path(
        model_path=legacy_model, model_type=model_type, config_path=config_path
    )
...
@click.option(
    "--shard-size", default=10000, type=int, help="Number of samples per shard"
)
@click.option(
    "--shuffle/--no-shuffle",
    default=True,
...
@click.option(
    "--compression", default="gzip", help="Compression to use for output files"
)
...
@click.option(
    "--output-path", "-o", default="config.yml", help="Path to save configuration file"
)
def init_config(config_template, output_path):
    ...
    templates = {
        "deepsea": {
            ...
            "optimizer": {"name": "adam", "learning_rate": 0.001, "weight_decay": 1e-6},
            "scheduler": {"name": "cosine", "eta_min": 1e-6},
        },
        "danq": {
            "model": {
                "name": "danq",
