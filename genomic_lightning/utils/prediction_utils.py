...
def _load_uavarprior_predictions(file_path: str) -> Dict:
    ...
    try:
        if file_path.endswith(".npy"):
            ...
            raise ValueError(
                f"Unsupported file extension for UAVarPrior predictions: {file_path}"
            )
    except Exception as e:
...
def _load_fugep_predictions(file_path: str) -> Dict:
    ...
    try:
        if file_path.endswith(".h5") or file_path.endswith(".hdf5"):
            ...
            raise ValueError(
                f"Unsupported file extension for FuGEP predictions: {file_path}"
            )
    except Exception as e:
            ...
            if all_metadata or target_names:
                ...
                if target_names:
                    metadata_group.create_dataset(
                        "target_names",
                        data=np.array(target_names, dtype=h5py.special_dtype(vlen=str)),
                    )
...
def combine_predictions(
    prediction_files: List[str], output_path: str, format: str = "hdf5"
) -> str:
    """
        ...
        if file_path.endswith(".npy"):
            ...
            with h5py.File(file_path, "r") as f:
                if "predictions" in f:
                    ...
                    raise ValueError(
                        f"Could not find predictions dataset in {file_path}"
                    )
            ...
            for col in df.columns:
                if col.startswith("metadata_") or col in [
                    "sequence_id",
                    "chrm",
                    "pos",
                    "ref",
