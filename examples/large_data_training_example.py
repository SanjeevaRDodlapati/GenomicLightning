...
def generate_synthetic_sharded_data(
    output_dir, num_shards=10, samples_per_shard=1000, seq_length=1000, num_targets=919
):
    ...
    for shard_idx in range(num_shards):
        ...
        targets = np.random.randint(0, 2, (samples_per_shard, num_targets)).astype(
            np.float32
        )
...
def main(args):
    ...
    # Set up logger
    logger = TensorBoardLogger(save_dir=args.output_dir, name="chromdragonn")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train ChromDragoNN model on large genomic datasets"
    )

    # Data parameters
    parser.add_argument(
        "--data-dir", type=str, default=None, help="Directory with sharded data files"
    )
    parser.add_argument(
        "--split-file",
        type=str,
    ...
    parser.add_argument(
        "--num-shards", type=int, default=10, help="Number of shards to generate"
    )
    parser.add_argument(
        "--samples-per-shard",
        type=int,
    ...
    parser.add_argument(
        "--seq-length", type=int, default=1000, help="Length of input sequences"
    )
    parser.add_argument(
        "--num-targets", type=int, default=919, help="Number of prediction targets"
    )

    # Model parameters
    parser.add_argument(
        "--num-filters", type=int, default=300, help="Number of convolutional filters"
    )
    parser.add_argument(
        "--filter-sizes",
        type=str,
    ...
    parser.add_argument(
        "--residual-blocks", type=int, default=3, help="Number of residual blocks"
    )
    ...
    if not args.generate_data and (args.data_dir is None or args.split_file is None):
        parser.error(
            "Either --generate-data or both --data-dir and --split-file must be provided"
        )
