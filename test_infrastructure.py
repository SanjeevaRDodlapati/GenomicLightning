...
def test_package_imports():
    ...
    try:
        ...
        if hasattr(genomic_lightning, "__version__"):
            print(
                f"✅ GenomicLightning: Import successful, version {genomic_lightning.__version__}"
            )
        else:
    ...
    test_imports = [("fugep", "FuGEP"), ("uavarprior", "UAVarPrior")]
    ...
    for module_name, repo_name in test_imports:
        try:
            ...
            print(
                f"⚠️ {repo_name}: Import failed (expected - dependencies may be missing)"
            )
